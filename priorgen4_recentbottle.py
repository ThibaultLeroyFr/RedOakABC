...
priorgen.py generates prior distributions for multiple multilocus simulations under 8 main models of divergence.\
...
\tCamille Roux, Quentin Rougemont & Thibault Leroy: camille.roux.1@unil.ch & thibault.leroy@pierroton.inra.fr\n\t13/06/2016\n"
for i in sys.argv:
	if("help" in i):
		print(help)
		sys.exit(0)
...
for i in range(nreps):
	...
	Tbottleneck=uniform(taubottle[0],min(Tsmall, taubottle[1])) # a bottleneck after split
...
### pour tous les autres modèles (avec 2 pops filles)
	if(model=="SI" or model=="IM" or model=="AM" or model=="SC"):
	...
	for loc in range(nlocus):
		...
		if(model=="AM"):
			...
			if(bottleneck=="N"):
                        	cout+="{0} {1} {2} {3} {4} {5:.5f} {6:.5f} {7:.5f} {8:.5f} {9:.5f} {10:.5f} {11:.5f} {12:.5f} ".format(int(nspA[loc])+int(nspB[loc]), int(nspA[loc]), int(nspB[loc]), 0, 0, float(n1priorGenomic["values"][loc]), float(n2priorGenomic["values"][loc]), TsmallGenomic[loc], float(M1priorGenomic["values"][loc]), float(M2priorGenomic["values"][loc]),float(TsplitGenomic[loc]), float(TsplitGenomic[loc]), float(nApriorGenomic["values"][loc]))
		if(model=="SC"):
			if(bottleneck=="Y"):
